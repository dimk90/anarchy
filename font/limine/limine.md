...
>  - Control positions 0x00–0x1F and 0x7F are left blank (a boot menu never
>    needs them).
>  - A font only renders the CP437 glyphs it actually contains. Terminus and
>    Spleen cover box-drawing + blocks natively; **ProFont is ISO 8859-1**, so
>    its box-drawing/blocks are filled from a Terminus donor (below).
...
|---|---|---|
| `terminus-8x16.bin` | Terminus `ter-u16n` (8×16) | full CP437, box-drawing + blocks |
| `spleen-8x16.bin` | Spleen `spleen-8x16` (8×16) | full CP437, box-drawing + blocks |
| `profont-8x16.bin` | ProFont `r400-15` (7×15, padded) + Terminus donor | Latin from ProFont; box-drawing/blocks from Terminus |
...
- Get 8-wide BDF sources:
    - Terminus `ter-u16n.bdf` from the
...
- Convert each to Limine's raw format with [`bdf2limine.py`](bdf2limine.py). It
  ...
  merge as the PSF [ProFont build](../psf/profont.md):
    ```shell
    python3 bdf2limine.py ter-u16n.bdf        terminus-8x16.bin 16
    python3 bdf2limine.py spleen-8x16.bdf     spleen-8x16.bin   16
