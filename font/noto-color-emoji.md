...
- Excluded  `*`, `#` and keycap (`0`-`9`) emoji -> enable grouping with other non-emoji fonts.
- `SBIX` version for iOS/Mac compatibility.
  - Fixed `VS15` unicode modifier (B&W emoji version).
...
Install font building tool:
```shell
uv tool install "git+https://github.com/googlefonts/nanoemoji.git"
```
...
Copy build configs and start building:
```shell
cp anarchy/font/*.toml noto-emoji/
```

```shell
cd noto-emoji && uvx nanoemoji *.toml
```
